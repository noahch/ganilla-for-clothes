...
if opt_v == '01':
    ...
    opt_01 = Namespace(aspect_ratio=1.0, batch_size=1, checkpoints_dir='checkpoints/',
                       cityscape_fnames='./datasets/cityscapes_test_file_names.txt', cityscapes=False,
                       dataroot='../datasets/' + dataset, dataset_mode='unaligned', direction='AtoB',
                       display_winsize=256,
                       epoch=epoch, eval=False, fineSize=64, fpn_weights=[1.0, 1.0, 1.0, 1.0], gpu_ids=[0],
                       init_gain=0.02,
                       init_type='normal', input_nc=3, isTrain=False, loadSize=64, max_dataset_size=float("inf"),
                       model=model,
                       n_layers_D=3, name=experiment_name, ndf=64, netD='basic', netG='resnet_fpn', ngf=64,
                       no_dropout=True, no_flip=False, norm='instance', ntest=float("inf"), num_test=751, num_threads=8,
                       output_nc=3,
                       phase='test', resize_or_crop='resize_and_crop', results_dir='./results/', serial_batches=False,
                       suffix='', verbose=False)
    ...
    dataset = '10K'
    epochs_to_test = ['latest', '1', '15', '30']
    experiment_name = 'm05_' + opt_v + '_conv_deconv_' + dataset
    opt_03 = Namespace(aspect_ratio=1.0, batch_size=1, checkpoints_dir='checkpoints/',
                       cityscape_fnames='./datasets/cityscapes_test_file_names.txt', cityscapes=False,
                       dataroot='../datasets/' + dataset, dataset_mode='unaligned', direction='AtoB',
                       display_winsize=256,
                       epoch=epoch, eval=False, fineSize=64, fpn_weights=[1.0, 1.0, 1.0, 1.0], gpu_ids=[0],
